...
def day_tasks(year, month, day):
    ...
    if request.method == 'POST':
        if 'delete' in request.form:
            ...
            if new_task:
                cursor.execute('UPDATE tasks SET task = ?, time = ? WHERE id = ? AND user_id = ?',
    ...
    # GET-запрос
    cursor.execute('SELECT id, task, time, created FROM tasks WHERE user_id = ? AND year = ? AND month = ? AND day = ? ORDER BY time, created',
