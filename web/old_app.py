...
def day_tasks(year, month, day):
    ...
    if request.method == 'POST':
        if 'task' in request.form and 'task_id' not in request.form:  # Добавление новой задачи
            task = request.form.get('task')
            time = request.form.get('time')
            if task:
                cursor = db.cursor()
                cursor.execute('''
                    INSERT INTO tasks (user_id, year, month, day, task, time)
                    VALUES (?, ?, ?, ?, ?, ?)
                ''', (current_user.id, year, month, day, task, time))
                db.commit()
                flash('Задача успешно добавлена', 'success')
        elif 'delete' in request.form:  # Удаление задачи
        ...
        elif 'task_id' in request.form:  # Редактирование задачи
            ...
            if new_task:
                ...
                cursor.execute('''
                    ...
                    SET task = ?, time = ?
                    WHERE id = ? AND user_id = ?
                ''', (new_task, new_time, task_id, current_user.id))
                db.commit()
    ...
    cursor = db.cursor()
    cursor.execute('''
            SELECT id, task, time, created FROM tasks 
